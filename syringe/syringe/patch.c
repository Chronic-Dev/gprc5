...
int patch_firmware(unsigned char* address, int size) {
	...
	*/
	...
	if(cert_offset == NULL) {
		...
		unsigned char* permission_offset = patch_find(address, size, "\x00\x38\x18\xBF\x01\x20\x80\xBD", 8);
		if(permission_offset == NULL) {
			permission_offset = patch_find(address, size, "\x83\x43\xD8\x0F\x01\x23\x58\x40", 8);
			if(permission_offset == NULL) {
